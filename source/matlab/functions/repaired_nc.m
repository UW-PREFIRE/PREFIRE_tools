...
% **(modified slightly to repair/work_around a bug/feature in the original)

classdef repaired_nc < handle
    ...
    methods
        ...
        % File
        %------------------------------------------------------------------
        function this = repaired_nc(varargin)
            %NC(Filename)
            %NC(Filename, MODE)
            ...
            if  iscell(this.Format) 
                error(message('MATLAB:imagesci:netcdf:argumentWasNotChar',"Format"));
            end
            if ~any(strcmpi(this.Format, {'classic','netcdf4_classic','netcdf4','64bit',''}))
               error(message('MATLAB:imagesci:netcdf:badFormat'));
            end
        ...
        %------------------------------------------------------------------
        function writeAttribute(this, location, attName, attValue, varargin)
            ...
            %    LOCATION to '/'. ATTVALUE can be a numeric vector, a
            %    character vector or a scalar string. For netcdf4
            %    files, ATTVALUE can also be a vector of strings.
            %
            %    writeAttribute supports the following optional
            %    Name-Value Pair Argument:
            %
            %    'Datatype'   TYPE. The datatype of the NetCDF attribute. 
            %
            %         Use this           to create NetCDF attribute of type
            %         'double'               NC_DOUBLE
            %         'single'               NC_FLOAT
            %         'int64'                NC_INT64*
            %         'uint64'               NC_UINT64*
            ...
            p = inputParser;
            p.addParameter('Datatype', '', @(x)ischar(x) && ~isempty(x));
            p.parse(varargin{:});
            
            internal.matlab.imagesci.nc.validateDatatypeAgainstFileFormat(...
                p.Results.Datatype, this.Format);
            ...
            % no datatype provided, use default datatype mapping
            if isempty(p.Results.Datatype)
                netcdf.putAtt(gid, varid, attName, attValue);
            else % use specified datatype
                xtype = internal.matlab.imagesci.nc.dataClasstoxType(p.Results.Datatype);
                netcdf.putAtt(gid, varid, attName, attValue, xtype);
            end
        end
        ...
        function createVariable(this, varargin)
            ...
            p = inputParser;
            p.addParameter('Dimensions',{});
            p.addParameter('Datatype','double',@(x)ischar(x));
            p.addParameter('FillValue','NA',...
                @(f) ...
                isempty(f) || ...       % from ncwriteschema
                isscalar(f) || ...      % custom fill value.
                ischar(f));             % for 'NA' (library default),
                                        % 'disable' (nofillMode == true)
                                        % or for NC_STRING custom fill value
                              
            p.addParameter('ChunkSize',[],@(x)isnumeric(x));
            p.addParameter('DeflateLevel',[],...
            ...
            for dimInd = 1:length(dimNames)
                ...
                % Get the dimension path as well as dimension name because we need to
                % account for fully-qualified dimension paths when creating variables.
                [dimPath, dimName] = ...
                    internal.matlab.imagesci.nc.parseDimLocation(dimNames{dimInd});

                if ~isempty(dimPath)
                    % The dimension name contains a path. This means the dimension is
                    % defined at a different group level than this variable. Call getgid
                    % to get the group id associated with this path.
                    dimGid = this.getgid(dimPath, false);
                else
                    % Else the dimension is defined at the current group level, so we
                    % just need the current group id.
                    dimGid = gid;
                end

                dimids(dimInd) = netcdf.inqDimID(dimGid, dimName);
            ...
            if(this.isHDF5Based())
                ...
                if(strcmpi(fillValue, 'disable'))
                    ...
                    % Cannot disable fill values for NC_STRING,
                    % Throwing a nice error in high-level interface
                    if (xType == netcdf.getConstant("NC_STRING"))
                        error(message('MATLAB:imagesci:netcdf:turnOffFillValueString'));
                    end
                    
                    netcdf.defVarFill(gid, varid, true,[]);
                    
                elseif ~(isempty(fillValue) || strcmpi(fillValue,'NA'))
        ...
        function data = read(this,location, start, count, stride)
            ...
            if ~internal.matlab.imagesci.nc.isDatatypeNumeric(vinfo.Datatype)
                % return data as-is, no attribute conventions for 
                % non-numeric data
                return;
            end
        ...
        function write(this, fullVarName, varData, start, stride)
            ...
            if strcmpi(varinfo.Datatype,'char')
                varData = convertStringsToChars(varData);
            elseif strcmpi(varinfo.Datatype,'string')
                % do not allow character matrices
                if ischar(varData) && ~isvector(varData)
                    error(message('MATLAB:imagesci:netcdf:charMatrixString'));
                end
                
                varData = convertCharsToStrings(varData);
            end

            % Attribute conventions (for numeric types)
            if ~isempty(varinfo.Attributes) && ... % some attributes are present
                    strcmpi(this.Conventions,'default') &&...
                    internal.matlab.imagesci.nc.isDatatypeNumeric(varinfo.Datatype)
        ...
        function info = info(this, location)
            ...
            %    Datatypes: An array of structures with these fields
            %                 Name:           Name of User-Defined type
            %                 Class:          Class of User-Defined type
            %                 Type:           Base type of NC_VLEN 
            %                 ByteSize:       Size in bytes of User-Defined type
            %
            ...
            % Add Datatypes field for User-Defined Types
            if strcmp(location, '/')
                info.Datatypes = this.userTypeInfo();
            end
        end
        ...
        function disp(this, location)
            ...
            if(nargin==1)
                ...
                if(varid == -1)
                    ...
                    this.dispUserTypes(this.userTypeInfo(), '');
    ...
    methods (Hidden=true, Static=true)
        function this = loadobj(sobj)
            ...
            this = repaired_nc(sobj.Filename);
        end
    end
    ...
    methods (Access=protected)
        ...
        function vinfo = varInfo(this, gid, varids)
            ...
            for vInd = 1:length(varids)
                ...
                vinfo(vInd).Datatype = this.typeIDToDatatypeName(xType);
        ...
        function dinfo = dimInfo(~, gid, dimids, unlimdimids)
            ...
            % Keep track of unlimited dimensions in parent group.
            parentGrpUnlimDimIds = [];
            ...
            for dInd = 1:length(dimids)
                ...
                while(~ismember(dimid, dimIDsDefinedInThisGroup))
                    ...
                    parentGrpUnlimDimIds = netcdf.inqUnlimDims(dimDefinedGrpID);
                end
                ...
                % is this an unlimited dimension in this group or the parent group?
                if ismember(dimid,unlimdimids) || ...
                    (~isDimLocallyDefined && ismember(dimid,parentGrpUnlimDimIds)) 
        ...
        %--------------------------------------------------------------------------
        %Get info about User-Defined Types in the file
        function udInfo = userTypeInfo(this)
            udInfo = [];
            typeID = netcdf.getConstant('NC_FIRSTUSERTYPEID');

            % Starting with NC_FIRSTUSERTYPEID, keep going until
            % there are no more user-defined types
            while true
                try
                    [typeName, byteSize, baseTypeID, ~, classID] = ...
                        netcdf.inqUserType(this.ncRootid, typeID);
                catch ME
                    % this typeID does not exist, stop
                    if strcmp(ME.identifier, 'MATLAB:imagesci:netcdf:libraryFailure')
                        break
                    end
                end

                % what kind of User-Defined Type? Capture info only for
                % supported types
                if classID == netcdf.getConstant("NC_VLEN")
                    udInfo(end+1).Name = typeName;
                    udInfo(end).Class = 'NC_VLEN';
                    udInfo(end).Type = this.typeIDToDatatypeName(baseTypeID);
                    udInfo(end).ByteSize = byteSize;
                end
                typeID = typeID+1;
            end
    ...
    methods (Access=protected)
        ...
        function writeDimensionSchema(this, schema, fullGroupName)
            ...
            % structure. The schema structure has Name, Length, Unlimited,
            % and Format fields with Unlimited and Format as optional.
            % schema can be an array of structures.
            
            if(nargin<3)
                fullGroupName = '/';
            end

            % Handle string support for the dimension schema.
            % The incoming dimension schema is a struct containing only dimension
            % name, length, unlimited flag, and file format, with the latter two
            % being optional. Since only name and format can be character vectors
            % or string scalars, and since the dimension struct doesn't contain
            % any attributes, we can convert strings to character vectors in the
            % whole struct to allow string support for dimension name and format. 
            schema = matlab.io.internal.imagesci.convertStringsInStruct(schema);
        ...
        function writeVariableSchema(this, schemaStructs, fullGroupName)
            ...
            for infoInd = 1:length(schemaStructs)
                ...
                % Handle string support for the variable schema.
                % The incoming variable schema can contain attribute values and fill
                % values which we don't want to convert from string to character vectors
                % because they are allowed to contain string values. Thus we selectively
                % convert just the variable name, datatype, and file format to
                % character vectors. Note format is optional.
                schema.Name = convertStringsToChars(schema.Name); 
                schema.Datatype = convertStringsToChars(schema.Datatype); 
                if isfield(schema,'Format')
                    schema.Format = convertStringsToChars(schema.Format);
                end
                ...
                for aInd = 1:length(schema.Attributes)
                    ...
                    % if attribute is a string, make sure to write it as
                    % such (scalar strings are written as chars by default)
                    if strcmpi(class(schema.Attributes(aInd).Value), 'string')
                        this.writeAttribute([fullGroupName schema.Name],...
                            schema.Attributes(aInd).Name, ...
                            schema.Attributes(aInd).Value, ...
                            'Datatype', 'string');
                    else
                        % otherwise use default mapping from matlab class
                        % to netcdf type
                        this.writeAttribute([fullGroupName schema.Name],...
                            schema.Attributes(aInd).Name, ...
                            schema.Attributes(aInd).Value);
        ...
        function writeGroupSchema(this, schemaStructs, parentGroup)
            ...
            % Handle string support for the group schema.
            % Since the incoming group schema can contain attribute values
            % which we don't want to convert to character vectors, we selectively
            % convert the group name and format to character vectors. Note format
            % is optional.
            if length(schemaStructs) > 1
                for isa=1:length(schemaStructs)
                    schemaStructs(isa).Name = ...
                                convertStringsToChars(schemaStructs(isa).Name); 
                    if isfield(schemaStructs(isa), 'Format');
                       schemaStructs(isa).Format = ...
                              convertStringsToChars(schemaStructs(isa).Format); 
                    end
                end
            else
                schemaStructs.Name = convertStringsToChars(schemaStructs.Name); 
                if isfield(schemaStructs,'Format');
                    schemaStructs.Format = ...
                                   convertStringsToChars(schemaStructs.Format); 
                end
            end
            ...
            knownFields        = {'Name','Dimensions','Variables',...
                'Attributes','Groups','Filename','Format','Datatypes'};
            allFields          = fieldnames(schemaStructs);
            unRecognizedFields = setdiff(allFields, knownFields);
            if(~isempty(unRecognizedFields))
                unRecognizedFieldStrings = ...
            ...
            % If there are any User-Defined Datatypes (currently, can only
            % be NC_VLENs), error out as ncwriteschema does not support them.
            if isfield(schemaStructs,'Datatypes') && ~isempty(schemaStructs.Datatypes)
                error(message("MATLAB:imagesci:netcdf:ncwriteschemaVLEN"));
            end
                ...
                if(isfield(schema,'Attributes') && ...
                    ...
                    for aInd = 1:length(schema.Attributes)
                        ...
                        if strcmpi(class(schema.Attributes(aInd).Value), 'string')
                            ...
                            % otherwise use default mapping from matlab
                            % class to netcdf type
                            this.writeAttribute(schema.Name,...
                                schema.Attributes(aInd).Name,...
                                schema.Attributes(aInd).Value);
                        end
    ...
    methods(Access=protected)
        ...
        %--------------------------------------------------------------------------
        %Display all supported User-Defined Types in the file
        function dispUserTypes(~, tinfo, indentSpace)
            if(isempty(tinfo))
                %nothing to do
                return;
            end
            userDefinedTypeMsg = getString(message('MATLAB:imagesci:netcdf:userDefinedTypesDisplay'));
            fprintf('%s%s\n',indentSpace,userDefinedTypeMsg);

            % for each user-defined type
            for tInd = 1:length(tinfo)
                fprintf('%s           %s: {%s}\n',indentSpace, tinfo(tInd).Name,...
                    tinfo(tInd).Type);
            end
        ...
        function dispncAtts(~, ainfo,indentSpace)
            ...
            %Obtain display string for each of the attributes
            for aInd = 1:length(ainfo)
                attDesc{aInd} = ...
                    internal.matlab.imagesci.nc.dispStringAttValue(...
                    ainfo(aInd).Value, attValuePadSpace);
            end
    ...
    methods (Access=protected)
        ...
        %------------------------------------------------------------------
        % Obtain the string representation of a supported type. For atomic
        % type, it is the MATLAB datatype name. For supported User-Defined
        % Types, it is the type name given by the user.
        function typeName = typeIDToDatatypeName(this, typeID)

            % try to find atomic type name first
            typeName = internal.matlab.imagesci.nc.xTypetoDatatype(typeID);

            % could be a User-Defined Type
            if strcmpi(typeName, 'UNSUPPORTED DATATYPE')
                try
                    [userTypeName,~,~,~, classID] = netcdf.inqUserType(this.ncRootid, typeID);
                    % Is it a supported User-Defined type?
                    if classID == netcdf.getConstant('NC_VLEN')
                        typeName = userTypeName;
                    end
                catch
                    % do not re-throw the error, but return 'UNSUPPORTED DATATYPE'
                    % if the type was not a supported User-Defined Type in this file
                end
            end
    ...
    methods (Static=true, Hidden=true)
        ...
        %--------------------------------------------------------------------------
        % Display string for an attribute value
        % (is used by NetCDFImportProvider for the NetCDF Live Task)
        function dispStr = dispStringAttValue(value, attValuePadSpace)
            if isnumeric(value)
                if numel(value)>1
                    % Use [] to scope vectors
                    dispStr = ['[' num2str(value) ']'];
                else
                    dispStr = num2str(value);
                end
            elseif strcmp(value,'UNSUPPORTED DATATYPE')
                dispStr = value;
            elseif isstring(value) % NC_STRING
                % put strings in quotes
                dispStr = join('"' + value + '"');
                if numel(value)>1
                    % use [] for a vector of strings
                    dispStr = "[" +  dispStr + "]";
                end
            ...
            elseif ischar(value) % NC_CHAR
                ...
                dispStr = strrep(dispStr,...
                    newline,...
                    sprintf('\n%s           ',attValuePadSpace));
            elseif iscell(value) % NC_VLEN
                % if 0-element NC_VLEN data, display {}
                if isempty(value)
                    dispStr = "{}";
                else
                    % will recursively call dispStringAttValue to correctly
                    % display the NC_VLEN's base type elements
                    cellValues = cellfun(...
                        @(x)internal.matlab.imagesci.nc.dispStringAttValue(...
                        x(:)', attValuePadSpace), ...
                        value, 'UniformOutput', false);
                    cellValues = string(cellValues);

                    % make sure that empty or single NC_VLEN values are also
                    % surrounded by []
                    emptyOrSingleInds = ~contains(cellValues, "[");
                    cellValues(emptyOrSingleInds) = "[" + cellValues(emptyOrSingleInds) + "]";

                    dispStr = "{" + join(cellValues) + "}";
                end
            end
        end
        ...
        %--------------------------------------------------------------------------
        % Given string representation of data type (MATLAB datatype name or the name
        % of User-Defined type) return true if it is an atomic numeric type.
        function tf = isDatatypeNumeric(datatype)
            switch datatype
                case {'double', 'single', ...
                        'int8', 'uint8', ...
                        'int32', 'uint32', ...
        ...
        function xType = dataClasstoxType(dataClass)
            ...
            switch dataClass
                ...
                case 'string'
                    xType = netcdf.getConstant('NC_STRING');
                otherwise
        ...
        %------------------------------------------------------------------
        %Obtain the MATLAB data class from the NetCDF atomic datatype 
        ...
        %------------------------------------------------------------------
        % Check if a datatype can be used with this file format
        % (Some types are only allowed in NetCDF-4 files)
        function validateDatatypeAgainstFileFormat(datatype, fileformat)

            % If this file is not a netcdf4 file
            if(~strcmpi(fileformat,'netcdf4'))
                % Then we do not support creating some datatypes.
                switch datatype
                    case {'int64','uint64','uint32','uint16','uint8','string'}
                        error(message('MATLAB:imagesci:netcdf:unSupportedDatatype', datatype, fileformat));
                    otherwise
                        % all ok
                end
            end
